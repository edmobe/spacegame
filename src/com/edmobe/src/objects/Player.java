...
public class Player extends GlobalPosition{
	...
	private String playerimage = "/images/spaceship.png"; // player image path.

	int velX = 0; // x axis speed.
	int velY = 0; // y axis speed.
	
	boolean right = false; // indicates that the player is moving right.
	boolean left = false; // indicates that the player is moving left.
	...
	public void update(){ // update player's changing parameters.
		if (x == 0 && left) {velX = 0;} // setting left side limit.
		else if (x == 590 && right) {velX = 0;} // setting right side limit.
	...
	public void keyReleased(KeyEvent event) { // when a key is released.
		...
		if (key == KeyEvent.VK_RIGHT) { // if the right button is released.
			if (left == false) {velX = 0;} // if the player is not going left, the player stops.
			right = false; // the player is not moving right anymore.
		} else if (key == KeyEvent.VK_LEFT) { // if the left button is released.
			if (right == false) {velX = 0;} // if the player is not going right, the player stops.
			left = false; // the player is not moving left anymore.
		}		
	}
	
	public void draw(Graphics2D g2d){
		g2d.drawImage(getPlayerImage(), x, y, null); // draws player image on the screen.
	}
